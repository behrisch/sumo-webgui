...
layout(location=1) in vec2   a_pos;         // per-instance world XY (driver narrows f64->f32)
...
out vec4 v_color;
void main() {
    // Convert SUMO navi-angle (CW from north, degrees) to math-frame radians
    // (CCW from +x).  Done in the shader so we don't pay it per vehicle on
    // the CPU and don't have to upload a separate cos/sin buffer.
...
void VehicleLayer::initGL(QOpenGLFunctions_3_3_Core* gl) {
    ...
    // f32 navi-degrees, one per instance; shader computes cos/sin.
    m_gl->glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(float), nullptr);
...
void VehicleLayer::setSnapshot(const SimSnapshotPtr& snap) {
    ...
    // Upload the packed Batch byte columns directly — no CPU repack.
    m_posVbo.bind();
    m_posVbo.allocate(snap->veh_positions.data(),
                      static_cast<int>(snap->veh_positions.size()));
    m_rotVbo.bind();
    m_rotVbo.allocate(snap->veh_angles.data(),
                      static_cast<int>(snap->veh_angles.size()));
    m_colVbo.bind();
