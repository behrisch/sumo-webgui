  ...
  frontend/
    src/
      ...
      layers/
        ...
        LaneBarsLayer.ts    # merged TLS-signal + stop-line bars (one LineLayer)
...
|---------|-------------|-------|
| **Stopping lines** ✅ implemented (cache v7) | `bytes lane_has_stopline` — u8 byte per lane (1 = stop line). The decision is per-outgoing-CONNECTION using the same link-state rule sumo-gui follows: a lane gets a stop bar if any outgoing connection has link state ∈ {`m` minor, `s` stop, `w` allway-stop, `=` equal}. TLS-controlled links are excluded (the merged `LaneBarsLayer` draws coloured runtime bars for those). Frontend (`LaneBarsLayer.ts`) derives the two endpoints from the lane's last shape point + perpendicular × half-width, converting the perpendicular offset back to degrees in geo mode (÷ 111 000, ÷ cos(lat) for longitude). The same layer also renders TLS signal bars — both kinds are batched into one `LineLayer` with a per-bar kind discriminator for picking, and share the single "Signals & stop lines" toggle (`visibility.tls`). | Renders as a white bar across every uncontrolled minor approach, matching sumo-gui. v7 widens the rule from the v5/v6 junction-type list to the per-connection link state. |
